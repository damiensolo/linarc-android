...
- **Conversation is one primitive** (2026-09-18): a `CollabTopic` either stands alone or is *the* discussion for one object via `CollabSubject` (record, task, plan pin, photo; at most one topic per subject). Object detail screens embed `DiscussionSection` (records and tasks live; pins and photos next) and the topic is created lazily on the first message, so the Collaboration tool is the index of every thread. Messages post as the Demo lens identity (`CollabRepository.authorIdentity()`: Hector in Crew view, Sam in Subcontractor view, else the Foreman) and each Send queues one Outbox entry — never add a second comment model or a batch-publish step beside this.
