...
- Voice note (bilingual EN/ES voice capture → creates Issue/Incident/Punch item) and Issue stay one tap away as quick chips on the camera screen; they must remain usable even when camera permission is denied. The Voice note chip replaced the Voice daily log chip on 2026-08-25 — do not re-add voice daily log to the Capture UI. Review is editable; recording has Pause/Resume. Shared **Speak** is long-text only (record Description and Blocking reason, and every discussion composer via `DiscussionComposer`) — never a mic on every field; sequential with the camera (one in-app take at a time). Keyboard/IME voice typing remains the fallback on focused fields. **Speak on forms is opt-in and off by default** (2026-09-04): Settings → Voice input → "Voice input on forms" (`DemoProjectRepository.speakOnForms`); when off, `SpeakableTextField` is a plain text field. Voice note on Capture is never gated by that setting.
...
- Captured photos must land on Today, as Plan pins, and in the Images grid — never only in a private list. The full-screen photo viewer is a pager over the whole Images set (swipe between photos like the plan sheet viewer; per-page zoom) — do not regress it to a single-photo screen (2026-09-04). From a **plan pin** the viewer is scoped to pinned photos only (`ImageViewerScope.PLAN_PINS`, 2026-09-09) — never let a pin's photo swipe into unrelated album shots.
