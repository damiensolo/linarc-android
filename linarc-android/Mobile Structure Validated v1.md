...
Internal strategy prototype: prove a field-first information architecture on Android, with Voice-to-Log as the working differentiator. The app opens as a **Foreman**. All six other personas — **Superintendent**, **Crew**, **Project manager**, **Project engineer**, **Owner**, and **Subcontractor** — are live via Settings → Demo: view as (five added 2026-08-25 as the iteration-2 personas; Project engineer added 2026-09-03).
...
| **Plan** | Area B sheets + pins for issues, photos, and logs. Same list from the Plans tab or **Tools → Plans**. Not a PDF engine. |
...
- **Pattern B — nested browsing stack** (the built Tools areas, the remaining tool placeholders, voice log history, outbox, settings): **keeps the navigation bar visible**. Back moves one level at a time, and reselecting the active tab returns that tab to its root screen. Each tab is a nested graph, so tab stacks are preserved independently.
- **Pattern C — modal bottom sheet** for compact contextual actions or one-or-two parameter changes (Profile, new time entry, new topic, image source, photo discussion). Dismissible via scrim or swipe.
...
- New personas beyond the seven in the table (six went live 2026-08-25; Project engineer was sanctioned and built 2026-09-03 — an eighth starts as a view-as placeholder, not a build)
...
**Every Today list section is collapsible** (2026-09-03): section headers use the same treatment as the crew roster (`FieldCollapsibleSectionHeader` — title, live count, chevron; TalkBack reads the expanded state). Sections default expanded; the demoted "On site today" roster keeps its collapsed default. Expansion resets on persona switch. The Owner decision dashboard keeps its card layout — it has no list sections to collapse.
...
| **Crew** | **Live (view as, 2026-08-25)** | My assignment, start/end shift, my discussions, take a photo — see Crew UX below |
| **Project manager** | **Live (view as, 2026-08-25)** | Aging RFIs, delays, decisions — see Project manager UX below |
| **Project engineer** | **Live (view as, 2026-09-03)** | RFI desk (draft + chase), coordination & quality queue, decision threads — see Project engineer UX below |
...
- Area B sheet set (Compose drawings, not a PDF), searchable and grouped by discipline. The **Tools → Plans** card opens this same list (Pattern B, Back to the catalog) — not a placeholder and not a second module.
- Seed pins plus pins created from voice/photo/video/issue.
- Issue “near column 4” from the Hector script lands on the Column 4 pin (Level 2).
- Tap pin → `ModalBottomSheet` with title, snippet, and — when the pin came from a capture — the photo itself (tap to open the full-screen viewer). Every pin carries a **Discussion** — the same thread as the record or photo behind the pin (a pin is a location for something, not a thing of its own; only log/video pins get a thread of their own), rendered by the shared `DiscussionSection`. Add messages as the lens identity (or **Speak** into the composer); each **Send** queues one Outbox entry (offline-first; nothing leaves the device). The batch **Publish to team** step was retired 2026-09-18. Photos carry the same discussion behind a caption chip ("Discuss" / "N comments") in the full-screen viewer, opening a Pattern C sheet.
...
1. **Photo** — in-app CameraX camera (rear/front flip, flashlight, tap-to-focus, pinch-zoom) → review with title, description, and tags (suggested chips plus a search-or-add field, added 2026-09-03: type to find any existing project tag or mint a new one — suggestions are no longer the only option) → Save, or “Save & create…” which picks a record category (issue / incident / punch item) and continues into that record form with the photo already attached and the fields seeded. Photos land on Today, as Plan pins, and in Images. A **Markup** toggle chip on the viewfinder routes the shot through the annotation editor (select/move/resize, pen, line, arrow, double arrow, box, oval, cloud, text, six colors, undo/redo) before review; annotations are baked into the saved JPEG.
2. **Voice note** (quick chip on the camera; replaced the Voice daily log chip 2026-08-25) — bilingual voice capture as a create workflow. Recording shows the transcript live with an English | Español segmented toggle; the spoken language is auto-detected best-effort (the recognizer re-arms in the detected language) and one tap overrides it. **Pause / Resume** keeps the words already captured (stop then start without clearing the transcript). Done → review with a floating toolbar in the image-viewer style: **Share / Translate / Re-record / Delete / Create**. The review transcript is an **editable field** (type to fix a mis-heard word; Re-record for a new take). Translate (and the same toggle) flips the note between English and Spanish via on-device ML Kit translation — models download once, then it works offline. Review preselects Issue / Incident / Punch from spoken keywords (changeable chips) and pre-checks **Blocks work** when they said the work is stopped; Create still never auto-saves. Sequential **Add photo** (camera, then back — never dictation and shutter at once) attaches the shot to the forthcoming record; photo review can continue into Voice note the same way. **Add to…** files the note onto an existing task or record when the transcript names it (e.g. RFI-118, the med-gas task) instead of minting a duplicate. Create always opens the full form (the short-confirm quick save was retired 2026-09-03 with the derived title it depended on). The description is **exactly the text showing when Create was tapped (original or translation — never both merged)**; the **title is never prefilled** from dictated text — the reporter types it. The note itself is ephemeral: the records (or appends) made from it are the durable artifacts, and Create is a **full handoff** — the note pops off the back stack. No audio file is recorded. Works without camera permission.
3. **Issue** (quick chip on the camera) — opens the record create form (Issue category); the dictated-video flow prefills it via the parsed location/description (never the title — 2026-09-03). Description and Blocking reason have **Speak**; title does not.
...
**Today (Crew)** — focus: my assignment, start/end shift, my discussions, take a photo:
...
- **My discussions** (2026-09-18): the threads Hector takes part in — his tasks' discussions and anywhere he was `@`-mentioned (`TopicFilter.MINE` for the lens identity), most recently active first; rows open the conversation the Collaboration tool owns. Messages Hector sends post as Hector, not as the signed-in Foreman.
...
## Project engineer UX (view as, added 2026-09-03)

The seventh persona, sanctioned 2026-09-03 (superseding the earlier "six personas" non-goal). Target: a Project Engineer with a civil engineering / construction management / architectural engineering or equivalent technical construction background — the project's **technical document hub**. Where the PM reads the RFI list as an aging overview, the PE **works** it. Same rule as every persona: **same three tabs, same objects, reordered** — no new tabs, camera untouched, nothing removed (only Owner removes), no borrowed identity (like the Superintendent and PM, the view is a role lens; the profile stays Alex Rivera).

**Jobs to be done:** (1) turn field questions into tracked RFIs and chase the answers; (2) triage the coordination/quality records behind those questions; (3) keep drawings and project files current as references; (4) follow the A/E decision threads; (5) watch blockers whose resolution is technical.

**Today (Project engineer)** — focus: the RFI desk and its technical queue:

- **RFI desk** card leads (the PE's day ritual, mirroring Crew's My shift / the Sub's Inspections card): the status line reads the live queue — open-RFI count plus the oldest age via `rfiAgeLabel` ("2 open · oldest 6 days open"). **Draft RFI** stages the Issue create form **already on the RFI type** (`RecordDraft.begin(seedType = RFI_ISSUE_TYPE)` — a seeded type routes through `selectType`, so type blocking defaults apply exactly as a manual pick; an invalid seed is ignored). Saving publishes like any issue: Issues tool list, Plan pin, ONE queued Outbox entry.
- **Open RFIs**: the same `agingRfis()` queue the PM leads with (oldest first, age on the row, error-colored at 2+ days), because for the PE it is the working list, not an overview. Rows open the record detail the Issues tool owns.
- **Coordination & quality**: the technical records behind the questions — non-RFI issues plus punch items, in `attentionOrder()` via the pure `technicalQueue()` (`RecordModels.kt`). RFIs are excluded (they live in the section above); incidents are excluded (safety reporting is the Superintendent's oversight, not the PE's technical queue). Rows open record detail.
- **Blockers**: unchanged shared section (default label — a PE reads a stoppage as the technical constraint it is).
- **Decisions & discussions**: shared with the PM — the seeded med-gas and headwall threads ARE the conversations behind the two seeded RFIs.
- **Recent captures**: unchanged. **On site today**: roster demoted and collapsed, same as the other oversight personas.

**Tools (Project engineer)**: same catalog reordered — RFIs, Issues, Plans, Punch list, Drive lead (the queue, the records behind it, the drawings, QC closeout, and the specs/submittal files). Nothing removed or disabled.

**Plan (Project engineer)**: unchanged in this pass.
...
Real dictation (`SpeechRecognizer` with an `EXTRA_LANGUAGE` hint per utterance) and real on-device translation (ML Kit `translate`, approved dependency 2026-08-25). Spoken-language auto-detect is a text heuristic (`VoiceNoteLanguageDetector`) — best-effort by design; the one-tap toggle is authoritative. Dictation must sound like one take: the recognizer's start tone plays once per session and every re-arm after it stays silent (media/system streams muted for the session and always restored — on stop, reset, fatal error, and dispose). Create must prefill the description — exactly the text showing when Create was tapped, original or translation, never both (simplified 2026-08-25) — plus what derives cheaply (location matched against either language). **Never the title** (decided 2026-09-03): a transcript head made a junk title the reporter had to delete before typing the real one, so dictated text seeds only the description across every voice/dictation capture path (voice note, and the video flow's File an issue) until title inference is genuinely smarter (better parsing or AI). The reporter types the title on the form's required, empty title field. A missing translation falls back to the original; a create is never blocked on translation.

**Dictation reliability contract (2026-09-03):** transient recognizer errors (`ERROR_CLIENT`, `ERROR_RECOGNIZER_BUSY` — routinely fired by any stop/re-arm race, including the language toggle and Pause/Resume) must self-heal: recycle the `SpeechRecognizer` and re-arm silently, capped at a handful of consecutive retries before surfacing a plain-language "tap Resume" message (never a raw error string). A manual language pick turns auto-detect **off** for the rest of the take (Re-record re-enables it) — detection must never flip the language back against the user's choice, and it reads only finalized-transcript growth, never character offsets into the mutating partial. Re-record **cancels** (not stops) the in-flight utterance so a late result can't resurrect discarded words; Pause **stops** it so in-flight words still land. Review edits re-translate automatically — the display toggle must never dead-end on a stale "couldn't translate" state.

Keyword routing (`VoiceNoteIntent`) is regex/vocabulary only — no Whisper/LLM. Hits preselect Issue vs Incident vs Punch and **Blocks work** (reason seeded from the transcript); the reporter confirms on review. The review body is editable (type to fix; Re-record for a new take). Recording **Pause / Resume** keeps the transcript. Photo attach is **sequential** (dictate then camera, or photo then dictate) so the mic is never contended — the same rule as form **Speak**: only one in-app dictation take at a time (`FieldDictationBroker`); opening the camera stops Speak. Speak is the gloves-sized path on long text only (record Description and Blocking reason, and every discussion composer); do not add a mic to title, chips, dropdowns, dates, or numeric hours. Keyboard/IME voice typing still works on focused fields. **Add to…** posts the note as a message in the matched open task's or record's **Discussion** (author and time kept — since 2026-09-18 it no longer appends raw text to the note or description; attached photos still attach to a record), queues one Outbox entry, and publishes a Today row that deep-links to that object — it must not dead-end. Create always opens the full create form (the short-confirm quick save was retired 2026-09-03 with the derived title it depended on — a record needs a title, and only the reporter provides one now; do not resurrect it without trustworthy title inference). Do not keep a voice-note inbox; do not merge both languages into the description; do not auto-file without showing the words.
...
2. Capture (bottom bar) → Voice note chip → speak Spanish ("falta concreto en la pared del nivel dos") → toggle auto-switches to Español with live text → Pause/Resume if needed → Done → type to fix a word if the recognizer missed it → Translate shows the English version → review chips preselect Issue → Create → the form arrives with the English text (the language on screen) in the description, Level 2 location, and an empty title → type the title (or Speak into Description / Blocking reason) → Save. Optional: Add photo before Create; or say "blocking plumbing at column 4" / "RFI-118" to pre-check blocking or Add to the named task/record.
...
8. Settings → Demo: view as → Project engineer → Today opens on the RFI desk (count + oldest age) → Draft RFI opens the Issue form already on RFI / design clarification → Save → the new RFI appears at the bottom of Open RFIs ("Opened today") and queues in the Outbox; below it, Coordination & quality lists the med-gas conflict first (blocking); Tools leads with RFIs, Issues, Plans (the Plans card is the live sheet list).
9. Settings → Demo: view as → Owner — Decision dashboard (v2) → Today opens on Progress photos, then the four decision topics (Schedule health · Budget & changes · Quality & approvals · Decisions needed), then Delays with the med-gas blocker's impact line; Tools leads with Images and Plans (live sheets), has **no Time card tile and no Voice logs row**.
10. Settings → Demo: view as → Subcontractor → Today opens on the Inspections card + My work (the blocked med-gas task in red); Request inspection → pick "Set fixture carriers" → the request lands on Today (tap it to open the task) and queues in the Outbox; Tools leads with Field task and Checklist.
11. Tools → Issues → Med-gas conflict at Column 4 → Discussion shows the same four med-gas messages as Tools → Collaboration; type `@Maria Chen`, Send → she joins the thread and one Outbox entry queues. Plans → Column 4 pin → the same thread. Tools → Images → any photo → Discuss chip → Send → "1 comment". Settings → Demo: view as → Crew → Today shows My discussions; sending posts as Hector.
