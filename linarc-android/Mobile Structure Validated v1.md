...
- Tap pin → `ModalBottomSheet` with title, snippet, and — when the pin came from a capture — the photo itself (tap to open the full-screen viewer). Every pin carries a comment thread: add comments as the signed-in user (or **Speak** into the comment field), then **Publish to team** queues the unpublished batch to the Outbox (offline-first; nothing leaves the device).
