...
| **Tools** | Platform modules (grid or list), then Appearance (theme), then demo controls last: view as, splash, outbox, voice log history. |
...
- Below the catalog: Appearance — Dark theme toggle (light and dark chrome).
- The Demo section sits last (it's strategy-demo scaffolding, not product surface): Demo: view as — Foreman selected and live; other personas visible (tapping a non-live persona explains that the view is next; it must not fake a broken UI); Splash animation picker; Outbox — every publish-style action (records, time entries, pin-comment publishes) queues here as "waiting for signal" (count only; no sync engine); Voice logs — history and playback of submitted recordings.
