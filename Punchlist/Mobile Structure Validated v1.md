...
3. **Issue** (quick chip on the camera) — opens the record create form (Issue category); the dictated-video flow prefills it via the parsed location/description (never the title — 2026-09-03).
...
8. Settings → Demo: view as → Project engineer → Today opens on the RFI desk (count + oldest age) → Draft RFI opens the Issue form already on RFI / design clarification → Save → the new RFI appears at the bottom of Open RFIs ("Opened today") and queues in the Outbox; below it, Coordination & quality lists the med-gas conflict first (blocking); Tools leads with RFIs, Issues, Plans.
9. Settings → Demo: view as → Owner — Decision dashboard (v2) → Today opens on Progress photos, then the four decision topics (Schedule health · Budget & changes · Quality & approvals · Decisions needed), then Delays with the med-gas blocker's impact line; Tools leads with Images and Plans, has **no Time card tile and no Voice logs row**.
