...
**Today (Owner)** — focus: **confidence, not operations** (upgraded 2026-08-25 from the first "photos + topic list" pass to a decision dashboard — `OwnerTodaySections.kt`, pure logic in `OwnerDashboard.kt`). **Both layouts stay demoable**: Settings → Demo → "Owner Today layout" switches between the v2 dashboard (default, below) and the original v1 view (photos → Decisions & discussions list → shared Delays rows → collapsed roster) for side-by-side demo conversations (`OwnerTodayVariant`, reset to dashboard on logout):
...
8. Settings → Demo: view as → Owner → Today opens on Progress photos, then the four decision topics (Schedule health · Budget & changes · Quality & approvals · Decisions needed), then Delays with the med-gas blocker's impact line; Tools leads with Images and Plans, has **no Time card tile and no Voice logs row**.
