...
2. Capture (bottom bar) → Voice note chip → speak Spanish ("falta concreto en la pared del nivel dos") → toggle auto-switches to Español with live text → Done → Translate shows the English version → review chips preselect Issue → Create → the form arrives with the English text (the language on screen) in the description, Level 2 location, and an empty title → type the title → Save. Optional: Add photo before Create; or say "blocking plumbing at column 4" / "RFI-118" to pre-check blocking or Add to the named task/record.
